...
// @version      2.2.0
// @description  Tự động trích xuất và tải toàn bộ ảnh từ các trang Flipbook/Sách trực tuyến thành file ZIP (Chống chặn F12, Auto-Retry, Resume & Auto-Layout Fix)
...
(function () {
  ...
  function unlockInteractions() {
    const events = ['contextmenu', 'keydown', 'keyup', 'keypress', 'selectstart', 'dragstart', 'copy', 'cut', 'mousedown'];
    events.forEach((evt) => {
      window.addEventListener(evt, (e) => e.stopPropagation(), true);
      document.addEventListener(evt, (e) => e.stopPropagation(), true);
    });
    const style = document.createElement('style');
    style.id = '__unblock_interactions_style__';
    style.textContent = `* { -webkit-user-select: auto !important; user-select: auto !important; pointer-events: auto !important; }`;
    if (!document.getElementById(style.id)) document.head.appendChild(style);
  }
  unlockInteractions();

  if (document.getElementById('__flipbook_downloader_host__')) {
    document.getElementById('__flipbook_downloader_host__').remove();
  }
  ...
  const PatternDetector = {
    ...
    analyze() {
      ...
      return { title: this.cleanFilename(document.title), pattern: bestPattern || (urlList[0] || ''), padding: detectedPadding, startPage: domPageInfo.current || 1, maxPage: detectedMaxPage };
    }
  };
  ...
  function loadJSZip() {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error('Không thể nạp JSZip'));
  ...
  const fallbackExts = ['.jpg', '.jpeg', '.png', '.webp', '.svg', '.JPG', '.PNG'];

  async function requestBinaryWithRetry(url, maxRetries = 4, onRetryLog = null) {
    let delayMs = 500;
    const doRequest = (targetUrl) => {
      return new Promise((resolve, reject) => {
        if (typeof GM_xmlhttpRequest !== 'undefined') {
          GM_xmlhttpRequest({
            method: 'GET', url: targetUrl, responseType: 'arraybuffer', timeout: 20000,
            onload: (res) => (res.status >= 200 && res.status < 300) ? resolve(res.response) : reject(new Error(`HTTP ${res.status}`)),
            onerror: () => reject(new Error('Lỗi GM_XHR')),
            ontimeout: () => reject(new Error('Timeout GM_XHR'))
          });
        } else {
          const controller = new AbortController();
          const timer = setTimeout(() => controller.abort(), 20000);
          fetch(targetUrl, { signal: controller.signal, mode: 'cors', cache: 'force-cache' })
            .then(res => { clearTimeout(timer); if (!res.ok) throw new Error(`HTTP ${res.status}`); return res.arrayBuffer(); })
            .then(resolve)
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const buffer = await doRequest(url);
        if (!buffer || buffer.byteLength < 500) throw new Error('Dữ liệu rỗng');
        return buffer;
      } catch (err) {
        if (attempt === maxRetries) throw err;
        if (onRetryLog) onRetryLog(attempt, delayMs, err.message);
  ...
  async function downloadPageWithCandidates(primaryUrl, pageNum, onLog) {
    ...
    if (matchExt) {
      const ext = matchExt[1], q = matchExt[2] || '', base = primaryUrl.substring(0, primaryUrl.lastIndexOf('.' + ext));
      fallbackExts.forEach(e => { if (e.toLowerCase() !== '.' + ext.toLowerCase()) candidates.push(`${base}${e}${q}`); });
    }
    let lastErr;
    for (let i = 0; i < candidates.length; i++) {
      if (i > 0) onLog(`🔍 [Trang ${pageNum}] Dò thử: ${candidates[i].split('/').pop()}`);
      try {
        return await requestBinaryWithRetry(candidates[i], 3, (att, d, msg) => onLog(`⚠️ Trang ${pageNum} retry ${att} (${msg})`));
      } catch (e) { lastErr = e; }
    }
    throw lastErr;
  }
  ...
  host.id = '__flipbook_downloader_host__';
  ...
  style.textContent = `
    *, *::before, *::after { box-sizing: border-box !important; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
    .panel {
      position: fixed; top: 20px; right: 20px; width: 410px; max-width: calc(100vw - 32px); max-height: calc(100vh - 40px);
      background: rgba(17, 24, 39, 0.96); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.15);
      border-radius: 14px; box-shadow: 0 20px 45px rgba(0,0,0,0.65); color: #F3F4F6; z-index: 2147483647;
    ...
    .close-btn { background: transparent; border: none; color: #9CA3AF; font-size: 18px; cursor: pointer; padding: 3px 7px; border-radius: 6px; }
    ...
    input[type="text"], input[type="number"], textarea {
      width: 100% !important; min-width: 0 !important; background: rgba(0, 0, 0, 0.45); border: 1px solid rgba(255, 255, 255, 0.14);
      border-radius: 8px; padding: 7px 10px; color: #F9FAFB; font-size: 12px; outline: none;
    }
    textarea { resize: vertical; word-break: break-all; min-height: 48px; }
    .row { display: flex; gap: 8px; width: 100%; } .row > .form-group { flex: 1 1 0; min-width: 0; }
    .btn-group { display: flex; gap: 6px; margin-top: 2px; width: 100%; }
    button.btn { flex: 1 1 0; min-width: 0; padding: 9px 8px; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; border: none; white-space: nowrap; }
    ...
    .progress-bar-fill { height: 100%; width: 0%; background: linear-gradient(90deg, #6366f1, #a855f7, #ec4899); border-radius: 3px; }
    .log-box { font-family: monospace; font-size: 11px; height: 75px; background: rgba(0, 0, 0, 0.55); border-radius: 6px; padding: 7px 10px; overflow-y: auto; color: #A7F3D0; white-space: pre-wrap; width: 100%; }
  ...
  const initialScan = PatternDetector.analyze();
  const panel = document.createElement('div');
  panel.className = 'panel';
  panel.innerHTML = `
    <div class="header" id="dragHeader"><div class="title">📖 Flipbook Downloader Pro</div><button class="close-btn" id="closeBtn">✕</button></div>
    <div class="body">
      <div class="form-group"><label>Tên tài liệu (.ZIP)</label><input type="text" id="docTitle" value="${initialScan.title}"></div>
      <div class="form-group"><div style="display:flex; justify-content:space-between;"><label>URL Mẫu (Pattern)</label><span class="badge">{page} hoặc {page:3}</span></div><textarea id="urlPattern" rows="2">${initialScan.pattern}</textarea></div>
      <div class="row">
        <div class="form-group"><label>Từ trang</label><input type="number" id="startPage" value="1"></div>
        <div class="form-group"><label>Đến trang</label><input type="number" id="endPage" value="${initialScan.maxPage}"></div>
        <div class="form-group"><label>Số luồng</label><input type="number" id="concurrency" value="4"></div>
      </div>
      <div class="btn-group">
      ...
      <div class="failed-box" id="failedBox">
        <strong id="failedCountTitle" style="color:#fca5a5;">⚠️ Trang bị lỗi:</strong>
      ...
      <div class="log-box" id="logBox">⚡ Đã kích hoạt (Responsive Layout OK)!</div>
    </div>
  `;
  ...
  dragHeader.addEventListener('mousedown', (e) => { isDragging = true; offX = e.clientX - panel.getBoundingClientRect().left; offY = e.clientY - panel.getBoundingClientRect().top; });
